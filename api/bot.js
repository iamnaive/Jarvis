...
// Runtime: Edge — fast ACK to Telegram, quick canned replies, delegate LLM to Node worker.
// Env required: TELEGRAM_TOKEN (or BOT_TOKEN), TELEGRAM_WEBHOOK_SECRET, INTERNAL_BEARER
// Optional: BOT_USERNAME (e.g. "jarviseggsbot", without @)
...
async function tg(method: string, payload: unknown) {
  try {
    ...
    if (!r.ok) {
      // swallow errors in Edge route (webhook must stay fast)
      await r.text();
    }
  } catch {}
}
...
export async function POST(req: Request) {
  if (!TG_TOKEN) return new Response("ok");
  if (TG_SECRET) {
  ...
  // Quick canned replies BEFORE any gating
  ...
  // Group gate (let passive replies only if we were not mentioned/replied/name-called)
  if (isGroup) {
    let pass = false;
  ...
  // Delegate to Node worker for LLM (fire-and-forget style)
  // We do a signed internal POST to avoid exposing keys via Edge route.
  try {
    ...
    await fetch(url.toString(), {
      ...
      body: JSON.stringify({
        ...
        threadId: msg?.message_thread_id ?? null, // for forum topics
      }),
      // don't await long; webhook must stay fast. Still we await fetch to ensure handoff.
    });
  } catch {}
